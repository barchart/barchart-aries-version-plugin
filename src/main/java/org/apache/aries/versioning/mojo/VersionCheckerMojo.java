...
extends AbstractMojo
{
  ...
  /**
   * Skip semantic vesioning check. 
   * 
   * @parameter expression="${skipSemanticVersion}" default-value="false"
   */
  private boolean skipSemanticVersion;

  /**
   * Major change in bundle version should override all rules for package version. 
   * 
   * @parameter expression="${bundleOverridesPackage}" default-value="false"
   */
  private boolean bundleOverridesPackage;

  //
  
  /** Verify if plugin execution should be ignored.*/
  private boolean shouldSkipMojo(){
	  if(skipSemanticVersion){
		  getLog().info("Configuration: skip semantic vesioning check.");
		  return true;
	  }
	  if("pom".equals(project.getPackaging())){
		  getLog().info("Packaging 'pom': skip semantic vesioning check.");
		  return true;
	  }
	  return false;
  }
  
  /** Verify if maven can resolve the artifact.*/
  private boolean canResolve(String artifact){
	  try{
		  File file = resolve(artifact);
		  return file.exists();
	  } catch(Throwable e){
		  getLog().warn("Baseline missing - skip semantic check.", e);
		  return false;
	  }
  }
  ...
  throws MojoExecutionException
  {
    ...
    if (oldArtifact != null && canResolve(oldArtifact)) {
      try {
        ...
        if(failedBundle){
        	...
        	if(bundleOverridesPackage){
            	getLog().warn("#################################");
            	getLog().warn("Bundle overrides package version.");
            	getLog().warn("#################################");
        	} else{
            	failed = true;
        	}
